...
| ~~23~~ | ~~**Stop suggestion in #19 was understated ~1.9x**~~ | **FIXED 2026-09-02** | Found live by Mandar reading the notification: it suggested `Stop ≈ -412` when the real net at that distance was **-936**. Two compounding errors: **(a)** it linearised a convex payoff, sampling the tangent 20 pts out and extrapolating to 79 — and sampling it beside the golden point where the curve is flattest, the worst place to linearise from (+20 pts costs ₹88, +79 costs ₹666); **(b)** it priced the stop on the exit-time curve when `/stradexit` compares the **running** net. Both reintroduce the very failure #19 was written to prevent: a number that looks like 79 points of protection and delivers ~45. Wrong in the direction that costs money. **Fix:** evaluate net directly at ±stop_pts on the running curve, take the worse side, and report the per-point figure as an average over the whole distance. **Regression test:** the naive version of the test failed — it asserted the two methods differ, which is position-dependent and passes trivially when spot is far from the peak. Rewritten to place spot **ON the golden point**, where the bug actually lived, and assert both that the stop equals the real net and that the tangent method would have understated it by >1.4x. **Also fixed a data-integrity defect found in the same session:** `_log_pit()` was added inside `_maybe_status()`, which `test_status_push.py` exercises — so every regression run appended its 08-20 fixture rows into the PRODUCTION `pit_snapshots.csv`. Seven leaked before it was caught; removed, and the suite now redirects `PIT_CSV` to a temp path. **Lesson: a test must redirect every side effect its target has acquired, including ones added after the test was written.** |
