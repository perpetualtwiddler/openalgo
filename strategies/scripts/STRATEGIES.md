...
| 23 | **Stop suggestion in #19 was understated ~1.9x — fix + regression test** | **TODO post-market 2026-09-02** | Found live by Mandar reading the notification: the push suggested `Stop ≈ -412` when the real net at that distance was **-936**. Two compounding errors, both mine, and both reintroduce the exact failure #19 was written to prevent — a number that looks like 79 points of protection and delivers ~45, back inside the noise band #16 exists to avoid. **(a) It linearised a convex payoff**, sampling the tangent 20 pts out and extrapolating to 79. The loss accelerates: +20 pts costs ₹88, +40 ₹232, +79 ₹666 — and the tangent was taken next to the golden point where the curve is flattest, the worst possible place to linearise from. **(b) Wrong time basis** — `/stradexit` compares the RUNNING net, so the stop must be priced at `T_now`; I used the exit-time curve, which is correct for the take-profit ladder and wrong for the stop. **Fix (written, deployed, NOT yet committed):** price the stop by evaluating net directly at ±stop_pts on the running curve and taking the worse side; report the per-point figure as an average over the whole distance rather than a peak-local tangent. Verified live: -412 → -936. **Still owed: a regression test.** It should assert the quoted stop equals the actual net at the stop distance to within a rupee or two — the property that fails under any linearisation. The take-profit ladder was correct throughout. **Note the deploy state:** the corrected file is on the server but the running strategy cached the old module at first import (~09:35), so today's pushes carry the understated figure; the fix takes effect at the next 09:15 spawn. |
| 22 | **openalgo stalls silently — UI unreachable while static assets still 200** | **WATCH (Mandar: park for now, 2026-09-02)** | First seen 2026-09-02: the login page and dashboard were unreachable from **both a phone on mobile data and a laptop** — so not DNS, not browser cache. `systemctl restart openalgo` fixed it instantly. **The diagnostic that WOULD have caught it, and that I missed:** `traffic_logs` stopped recording at **03:29:53** and logged nothing for five hours, where the previous day logged continuously (1,400–2,000/hr overnight, ~60/hr in session). The app had stopped doing work while still serving static files off disk. **Why my checks said 'fine' and were wrong:** I probed `/`, `/dashboard` and the JS/CSS bundles over the public hostname and got 200s — but those are static files that never touch the DB, session layer or broker. `systemctl` said active, Caddy logged no upstream errors, TLS and DNS were valid, ports open. Every signal I looked at was real and none of them tested whether the application was alive. **Lesson: 'static assets respond' is not 'the server is fine', and when the person at the browser says it is down, the browser is the better instrument.** Precedent we already had and I failed to reach for: the 2026-07 WS tick-delivery stall — openalgo can be `active` under eventlet, serve some paths, and be functionally wedged; fix is a restart. Single gunicorn worker + eventlet means one blocked greenlet can starve a subset of paths. **Note the timing:** the stall began ~03:29, close to Zerodha's ~03:00 daily token invalidation, and there is a scheduled restart at 09:05 that evidently did NOT clear it — worth checking whether that timer actually fired. **When we pick this up:** build a health check that exercises session/auth, a DB write and a broker round-trip, and alerts if `traffic_logs` goes quiet for >30 min during market hours. |
| 20 | **PIT dataset review — MID-MONTH: ~2026-09-15** | **⏰ SCHEDULED** | First look at `log/pit_snapshots.csv` after ~10 trading days (~120–140 rows). **Purpose is a data-quality check, NOT a rule yet.** Verify: rows land every check and the `kind` mix looks right (periodic/suppressed/on-demand); no blank or absurd values in the numeric core; the join to `trade_journal.csv` on date actually works; DTE coverage is spread rather than all 4–7. Then a first descriptive pass — does anything in the PIT state correlate with the day's eventual outcome at all? **Expect the answer to be 'not yet'** and resist fitting: ~10 days is fewer observations than the wing and stop sweeps had, and both of those produced a plausible winner that was noise. If the data is clean, the job is done; the temptation to build a rule on it is the failure mode. |
| 21 | **PIT dataset review — MONTH END: ~2026-09-30** | **⏰ SCHEDULED** | Second look, ~20–22 trading days (~250–300 rows). Now worth an actual test: does a PIT-conditioned exit beat the two baselines — (a) hold to the 15:00 square-off, and (b) Mandar reviewing the ladder and choosing? Baseline (b) is the one that matters and the one easiest to forget, since it is what we do today and it has been working. Method as per the wing/stop sweeps: state the hypothesis first, paired comparison on the same days, report the t-statistic and the noise band, and **be willing to conclude no**. The 14-Aug composition read (85% vega, Mandar's exit beat my hold recommendation by ₹859) is the one genuine signal we have and it is a single observation — so the specific thing to test is whether the durable/reversible split predicts anything, not whether some 25-variable combination can be fitted to 250 rows. |
| ~~7b~~ | ~~**Collect composition-at-decision data**~~ | **COLLECTING from 2026-09-02** | `straddle_analytics.pit_snapshot()` returns all 25 point-in-time variables; the strategy appends one row per status check to `log/pit_snapshots.csv` (27 cols, + `kind` and `lots`). **Recorded on EVERY check including suppressed ones** — suppression is about notification noise, not data value, and logging only the sent ones would bias the dataset toward days that moved. Previously the push computed 15 variables and logged 2, throwing every checkpoint away. This is the prerequisite for a dynamic exit rule (Mandar's stage 3): such a rule is a function from PIT state to action, and fitting one needs paired (state, outcome) observations — join this CSV to `trade_journal.csv` on date. ~10–14 rows/day means ~150–200 paired observations in a month. **The CSV is gitignored** — it is live position state and the mkds remote is a public fork. Guarded end-to-end: a missing expiry or an unwritable path is swallowed, since this shares a loop with the code enforcing PT/SL/breach. `test_pit_snapshot.py` also asserts the CSV row and the Telegram message agree on net/IV/durable/reversible/ceiling, because they are currently two computations and could otherwise drift. |
...
| ~~13~~ | ~~**Log SUCCESSFUL status pushes, not just suppressions**~~ | **DONE 2026-08-21** | `_maybe_status()` now logs `[STATUS] pushed (periodic|on-demand) — net …, IV …` on send, and `status_check.py --audit` counts pushes / suppressions / stale-rejections for a date, warns if >80% is being suppressed or if nothing is, and reads the strategy's own merged logs so there is no second counter to drift. **Takes effect from the next 09:15 spawn**, so 08-21 and earlier report an UNKNOWABLE send count rather than a misleading zero — the audit says so explicitly. First data point: 08-21 had **7 suppressions and 1 stale rejection**; with a ₹1,490 net range that day, if the send count turns out to be 1–2 the ₹400 threshold is arguably too quiet and worth lowering. Decide on 08-22's numbers, not on a guess. |
| 15 | **Retry entry later when 09:35 fails on AUTH alone** | MEDIUM | 2026-08-26: Mandar logged in late, so 09:35 returned `Incorrect api_key or access_token` on the gap check, the VIX check and the spot fetch. `_is_transient()` treated it as transient and retried 3× inside ~17s — all inside the same dead auth window — then set `entry_done_today` and forfeited the day. By 10:53 the session was fine. **Second occurrence** (27-Jul was the first, and cost one straddle win). The current retry policy is correct for a broker blip measured in seconds and useless for an auth gap measured in hours. Worth considering: if the ONLY failure is auth, and the session later recovers, re-attempt entry up to some cutoff (say 10:30) with the gates **re-evaluated at the new time** — an ORB check from 09:15–09:30 is meaningless at 10:55, so a late entry must be re-gated, not just re-fired. **Do not do this casually:** it widens the window in which we open a position, and every live day in the sample is a 09:35 entry, so a late-entry day is an untested trade. Quantify first from the captured chains — replay 09:35 vs 10:30 entries across the 68 days — before changing anything. |
| ~~17~~ | ~~**Should we trade 1 DTE at all?**~~ | **MEASURED 2026-09-01 — TRADE IT, but at HALF SIZE** | `dte_analysis.py`, 29 replayable days, our actual rules (breach + 15:00, no stop/target). **1 DTE has the HIGHEST mean (+470) and the best median (+1,315) of any bucket** — the opposite of what August's −₹6,185 over 4 days suggested. What it has is **1.70× the volatility** (stdev 3,022 vs 1,778); Welch t=+0.21 so the mean difference is NOT significant, but a variance ratio is estimated far more reliably than a mean, so 'trade it smaller' rests on the solid half of the finding. **Half size gives mean +235, worst −2,624 (vs −5,249), stdev 1,511.** **TIGHT STOPS ARE THE WRONG PRECAUTION and the data is emphatic:** on the 7 one-DTE days, stops at −1,000/−1,500/−2,000/−2,500/−3,000 give mean +244/−41/+146/−68/+3 and fire on **3–4 of 7 days, wrong 2–3 times**. 2026-06-29 (MAE −3,035 → finished +2,521) and 08-10 (MAE −1,841 → +1,315) are both killed by any tight stop, while the one day that needed protection (08-24, MAE −5,385 → −5,249) moved too fast for a stop to help. Cause is arithmetic not statistics: at 1 DTE ₹/point is highest, so a given rupee stop is the FEWEST points — ~68 pts against a 132-pt breach band. **Decision: 1 DTE at 1 lot, no stop, breach guard only.** Caveat: n=7. Oddity left unexplained: 6 DTE is the only negative bucket (−470, n=6). |
| ~~18~~ | ~~**`/straddle` command family**~~ | **DONE 2026-09-01** | `/straddle` (report) · `lots N` · `lots` · `skip` · `skip off`. Day-scoped in the same command file as `/stradexit`; all mutating forms are **refused after 09:35** with a reply saying why (by then the order exists and accepting would only pretend to change something). `skip` does NOT stop the strategy — it keeps monitoring and resumes tomorrow. **The `lots` refactor, and why it is a global mutation:** `QUANTITY` is read in 41 places. Rather than edit all 41 (where one miss silently trades the wrong size), `_apply_day_size()` reassigns the module globals ONCE, after `traded_dte` is known and before any order leaves, and is idempotent so the entry-retry loop cannot double-apply. That is only safe because **QUANTITY is never read at module scope** and LOTS only on its own init line — asserted by an AST walk in `test_straddle_cmds.py`, not assumed. **1-DTE days now auto-halve to 1 lot** (`DTE_HALF_SIZE=2`) per #17, with `lots N` as the override. 27 assertions. |
| ~~19~~ | ~~**Point-in-time exit RECOMMENDATIONS in the 30-min push**~~ | **DONE 2026-09-01** | `straddle_analytics.exit_advice()` + `format_exit_advice()`, wired into `format_status` so the periodic push and `/stradstatus` both carry it. Shows a **LADDER** (IV flat / −0.25pp / −0.50pp) rather than one number — the first draft computed `ceiling × 0.8 + shortfall` and suggested +550 on a day whose flat ceiling was +628 and which paid +841; showing what each level REQUIRES lets the reader price the assumption instead of inheriting mine. Adds the measured **₹70 fill shortfall**. The stop is in **NIFTY points as a fraction of the breach band** (60%), because a fixed rupee stop is ~68 pts at 1 DTE and ~151 at 7 DTE — the same number is a hair-trigger one day and unreachable the next. At ≤2 DTE it recommends **no stop, half size** per #17 instead of a number. Verified on two real positions: 7 DTE → ladder +887/+1,346/+1,822 with a −₹1,015 (79-pt) stop; 1 DTE → +1,047/+1,310/+1,576 and 'no stop, halve size'. |
| ~~16~~ | ~~**What stop / what target?**~~ | **MEASURED 2026-08-30 — never arm −1,500 again** | `exit_level_sweep.py` replays 29 captured days (0-DTE excluded) over a 6×6 grid of stop × target, first-rule-wins against breach and the 15:00 EOD. **−₹1,500 is the worst row in the table, in every target column** (mean −124/day vs +279 with no stop): it fires on **14 of 29 days** and was wrong on **9 of those 14, forgoing ₹16,373**. It is 0.79σ of our daily range — inside normal noise. **−₹2,000 strictly dominates it**: identical worst day (−2,759 both), +₹206/day better, fires 8 times instead of 14 — no trade-off at all. **Recommendation: −₹1,000 to −₹1,500 PER LOT (−₹2,000 to −₹3,000 at 2 lots).** **Targets: no standing default.** Every level from +750 to +2,500 sits inside the noise band and the ordering is non-monotonic — the signature of fitting. +750 lifts the win rate to 86% without making more money, which matches the earlier finding that fixed targets raise win rate and destroy return. Keep picking the number at runtime from the day's computed ceiling. **Honest limits:** no paired difference is significant (−1,500 vs none is t=−1.52, n=29); with stdev ₹2,083 anything under **₹774/day is noise**, so the claim is 'worst choice' and 'dominated', NOT 'the optimum is X'. Bar closes are not fills. The sample is dominated by 4–7 DTE; **1 DTE is a different animal** — 08-24's 106-point move would have blown through every level here, so at 1 DTE prefer a wider stop or none and rely on the 0.55% breach guard, which is the rule actually designed for directional moves. **Cost of not having measured this:** 2026-08-28's −1,500 fired at the day's worst minute; holding to the ordinary 15:00 exit would have turned −₹1,512 into **+₹1,500**, a ₹3,012 swing on one unmeasured parameter. |
| ~~14~~ | ~~**Is a fixed 400-point wing right across vol regimes?**~~ | **MEASURED 2026-08-25 — answer is NO CHANGE** | `wing_width_sweep.py` replays every captured day at 200/300/400/500. Excluding the 10 **0-DTE** days in the archive (we never sell the expiring series — leaving them in inflated every bucket), **28 days**: mean net **−51 / +124 / +243 / +334**. Monotonic in the OPPOSITE direction to the hypothesis, and it holds inside the 1–2 DTE bucket too (+47 → +610). Narrowing to 200 buys ~₹1,900 of tail protection (worst day −5,249 → −3,351) for **₹6,346/day** of extra hedge premium — you pay daily for protection that pays about one day in thirty, and at 200 the strategy is a **net loser**. **The premise was the error: risk:premium does not predict P&L.** At low DTE the 500pt wing has the WORST ratio (3.30) and the BEST return (+₹610) — the ratio weighs a defined worst case that almost never happens against premium collected every day. 08-24's ₹5,189 loss was therefore not a wing-width problem; the wings contributing ₹338 was them being CHEAP, not wrong. **Decision: keep 400.** Caveats kept honest: n=28, only 7 days at 1–2 DTE and 6 at 6+; bar closes are not fills so levels run optimistic; 500 beats 400 by ₹91/day which is well inside noise and NOT a reason to move. **Loose thread:** the 6+ DTE bucket is negative at every width (−370 to −616, n=6) — unexplained, worth a look when the sample grows. |
...
| ~~11~~ | ~~**Ingress allowlist on `/api/v1` (Caddy)**~~ | **DONE 2026-08-25** | Path-scoped, not a blanket deny — and the traffic logs are why. The external hits are Mandar's own browser driving the UI (66,548 `/api/v1/multiquotes`, 3,324 `/api/v1/positionbook`, plus `/auth/csrf-token` and `/sounds/alert.mp3`), and there has **never been a single order-placing call from an external IP** — `placeorder` appears only from 109.123.248.99 (323) and 127.0.0.1 (5). A blanket deny would have broken the remote UI while missing the actual hole. So the **13 transactional endpoints** (place/modify/cancel/close/basket/split/options/GTT) now `respond 403` unless `remote_ip` is 127.0.0.1, ::1 or the box itself; read-only stays open. **Deliberately does NOT allowlist Mandar's IP** — his ISP rotates it (39 distinct 106.215.x addresses since May, ~1/day), so an IP allowlist would lock him out constantly. **Why it's safe:** the strategy talks to `127.0.0.1:5000` directly and never through Caddy, so no Caddy change can affect live trading; and the UI Close/Close-All kill switch is `POST /close_position` (a session-authenticated blueprint at root), NOT `/api/v1/closeposition`. **Verified from a genuinely external IP:** all 13 transactional paths → `403 forbidden` from Caddy, read-only paths → 400 from the *app* (proof they proxied through), the same transactional paths → 400 from the app when called on-box, `/close_position` → 302 to login, UI paths 200. Also switched `reverse_proxy localhost` → `127.0.0.1` (the IPv6-preference trap recorded from the other box). Config checked into `deploy/caddy/Caddyfile`; server backup at `/etc/caddy/Caddyfile.bak-20260825`. **Threat closed:** SEBI static-IP whitelisting means a leaked API key is useless from an attacker's machine but perfectly usable routed THROUGH this box, which holds the whitelisted IP. |
...
| | IV −1pp | IV +1pp | net convexity |
|---|---|---|---|
| Short ATM straddle alone | +2,555 | −2,585 | **−30** — mildly adverse |
| Long wings alone | −693 | +890 | **+197** — positive vomma, as long options have |
| **Full iron butterfly (what we trade)** | **+1,862** | **−1,694** | **+168 FAVOURABLE** |

So the old claim — *"adverse convexity, a +1pp move costs more than −1pp gains"* — is true for a **bare** short straddle (the −30, and it is tiny) and **false for the structure we actually trade.** Vomma peaks out-of-the-money and is ~0 exactly at the money, so the two long OTM wings carry more of it than the sold ATM pair. A 1pp fall pays ₹168 **more** than a 1pp rise costs.

**Consequence for backlog #14:** the wings are not only a tail cap, they are also the source of this favourable vol convexity. Narrowing them to repair the risk:premium ratio would give some of it up — so "measure, don't change" gets stronger, not weaker.

**The caveat that keeps it honest:** this holds spot FIXED. Real vol spikes move spot too, and the joint move is what actually hurts — 2026-08-20 was IV +0.87pp **and** spot +42 off the strike, and no amount of favourable vol convexity saved that ₹1,881. Convexity in vol is not protection against gamma.

*Unit note, since it caused a question:* **pp = percentage point**, a change in IV, not a percentage of it. 8.77% → 9.77% is +1pp. Divide by ten for the intraday scale that matters: **0.1pp ≈ ₹184** on our size, versus ~₹70 for a whole hour of theta.
...
**⏰ Square-off moved 15:14 → 15:01 (2026-08-10) — NSE shortened the session.** ⚠️ **FACTUAL CORRECTION (2026-08-25): the F&O half of this is wrong.** The **cash/equity** segment ends 15:15 — which is why the NIFTY *index* freezes then, the index being computed from cash prices. **F&O keeps trading.** Verified across 3 symbols on 3 days: NIFTY option 1m candles run to at least **15:39** with real per-minute volume (50k–200k contracts, coherent OHLC, 1.8–13.1% of each day's total volume after 15:15). Zerodha's MIS auto-square-off for F&O at **15:26** is consistent with that and would be unexecutable otherwise. So the "~50 seconds of headroom" argument below rested on a false premise — 15:14 in fact had 25+ minutes. **The 15:00 exit stays regardless (Mandar, 2026-08-25): it is ~15 min clear of the cash close and well inside the most volatile window of the session, which is the real argument. Adjust at runtime with `/stradexit time`, do not re-litigate the default.** Consequence to remember if the question ever reopens: the old 45-day table below favouring 15:10/15:14 was dismissed as non-transferable "old-session" data — if the F&O session never changed, that data DOES transfer. Original (partly incorrect) text follows: Regular trading in stocks and F&O now ends **15:15** (was 15:30), effective **2026-08-03**. The old 15:14 exit had ~16 min of headroom before a 15:30 close; against a 15:15 close it left **~50 seconds**. That is not survivable: a clean 4-leg exit already takes ~7s (measured 2026-08-10, 15:14:03→15:14:10), and a transient failure needs a full retry cycle (8s × 2). 15:01 restores ~14 min of buffer.
