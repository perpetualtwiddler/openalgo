...
| 14 | **Is a fixed 400-point wing right across vol regimes?** | MEDIUM — measure, don't change | risk:premium has nearly doubled in four sessions: 0.78 (18-Aug, 7 DTE, straddle 267.85) → 0.82 → 1.14 → **1.29** (21-Aug, 4 DTE, straddle 187.10). Mechanical, not luck: `max_risk = 400 x qty − premium`, so as premium falls with vol and DTE the same envelope becomes a worse bargain — on 18-Aug we risked LESS than we collected, today we risk ₹29,321 to collect ₹22,679. Narrower wings in low vol would hold the ratio but cost more premium; the trade-off is unmeasured. `analyze_hedge_cost.py` is the tool. **Do not change the width on four days of drift** — this needs a proper sweep across the captured chains; the wings capped 2026-08-20's loss at 6.8% of defined worst case, AND (measured 08-21) they are what makes our net vol convexity favourable (+168) rather than adverse. Narrowing them costs both protections. |
