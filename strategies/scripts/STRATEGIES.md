...
| 18 | **`/straddle` command family** | **TODO post-market 2026-09-01** | Mandar's request. `/straddle lots X` sets the day's lot count (pre-09:35 only), `/straddle lots` reports configured vs actually-traded, `/straddle skip` skips today WITHOUT stopping the strategy (so it still monitors and resumes tomorrow), plus an un-skip. Same day-scoped command-file pattern as `/stradexit`. **`lots X` is the risky one:** `QUANTITY = LOT_SIZE * LOTS` is a module-level constant used in **41 places** — order placement, charge estimation, P&L, slippage, payoff projection, status message. Making it per-day means an instance attribute and touching every site; a single missed reference silently trades the wrong size. Needs a regression test asserting every order/charge/P&L path reads one value. All three must be no-ops after 09:35 with a clear reply saying so. This is also the mechanism for #17's half-size rule — consider defaulting to 1 lot on 1-DTE days with the command as an override. |
| 19 | **Point-in-time exit RECOMMENDATIONS in the 30-min push** | **TODO post-market 2026-09-01** | Mandar's request: each status notification should suggest a +ve and −ve exit number derived from live state, so the decision is guided rather than guessed. Draft rendered against the live 01-Sep position: reachable-by-15:00 at IV flat / −0.25pp / −0.50pp, a TP suggestion with the measured **₹70 fill shortfall** added, and an SL expressed in **NIFTY points as a fraction of the breach band** (60% ≈ 79 pts) rather than rupees — because a fixed rupee stop is 68 pts at 1 DTE and 151 pts at 7 DTE, a different rule every day. At ≤2 DTE it should recommend **no stop, half size** per #17 instead of a number. **Draft defects to fix before shipping:** the DTE label rendered empty, and the TP heuristic (`ceiling × 0.8 + shortfall`) suggested +550 when the IV-flat ceiling was +628 — cruder than just showing the three reachable levels and their requirement. Prefer presenting the ladder with what each needs over a single fabricated number. |
