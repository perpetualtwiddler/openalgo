...
| 13 | **Log SUCCESSFUL status pushes, not just suppressions** | **TODO post-market 2026-08-21** | Found live on the first day the feature ran. `_maybe_status()` logs `[STATUS] suppressed …` and `[STATUS] ignoring a stale request …`, but a push that actually goes out writes **nothing** — so the log cannot answer "did the 10:05 push fire?". Auditability has been the whole discipline on the journal and the charge model; the notification path should not be the exception. Add a one-line log on send, and a small daily audit (pushes vs suppressions vs on-demand) so we can tell whether `STATUS_NET_DELTA=400` / `STATUS_IV_DELTA_PP=0.15` fire at a sensible rate rather than assuming. |
| 14 | **Is a fixed 400-point wing right across vol regimes?** | MEDIUM — measure, don't change | risk:premium has nearly doubled in four sessions: 0.78 (18-Aug, 7 DTE, straddle 267.85) → 0.82 → 1.14 → **1.29** (21-Aug, 4 DTE, straddle 187.10). Mechanical, not luck: `max_risk = 400 x qty − premium`, so as premium falls with vol and DTE the same envelope becomes a worse bargain — on 18-Aug we risked LESS than we collected, today we risk ₹29,321 to collect ₹22,679. Narrower wings in low vol would hold the ratio but cost more premium; the trade-off is unmeasured. `analyze_hedge_cost.py` is the tool. **Do not change the width on four days of drift** — this needs a proper sweep across the captured chains, and note the wings are what capped yesterday's loss at 6.8% of worst case. |
